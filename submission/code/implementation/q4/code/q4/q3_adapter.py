...
def q3_upstream_refresh_audit_path() -> Path:
    """Locate the immutable Q3-to-Q4 refresh decision for either run root.

    A clean reproduction has its own output directory and deliberately starts
    empty.  The upstream-change decision, however, is formal input provenance
    rather than output generated by the partition search, so it is read from
    the formal result root when absent from the active clean-run root.
    """
    active = results_root() / "q4_q3_upstream_refresh_audit.json"
    if active.exists():
        return active
    return q4_root() / "results" / "q4_q3_upstream_refresh_audit.json"
