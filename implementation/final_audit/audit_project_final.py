...
def freeze_and_revision(data: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for q, final in (("Q1", "implementation/q1/results/q1_final.json"), ("Q2", "implementation/q2/results/q2_final.json"),
        ...
        revisions.append({"question": q, "branch": git("branch", "--show-current"),
                          ...
                          "exists_in_repository": True if git_at(source_root, "rev-parse", "--is-inside-work-tree") == "UNKNOWN" else bool((recorded and subprocess.call(["git", "cat-file", "-e", f"{recorded}^{{commit}}"], cwd=source_root, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) == 0) if recorded else True)})
...
def stale_and_paths() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    stale_result = {"status": "REVIEW_REQUIRED" if stale else "PASS", "files": stale, "deletion_performed": False}
...
def figure_table_manifests() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for q in ("q1", "q2", "q3", "q4"):
        qroot = qimpl(q)
        roots = [qroot / "figures", qroot / "results" / "figures"]
        for root in roots:
            if not root.exists(): continue
            for path in sorted(root.iterdir()):
                if path.is_file() and path.suffix.lower() in {".pdf", ".svg", ".tiff", ".png"}:
                    figures.append({"figure_id": path.stem, "question": q.upper(), "file": rel(path), "source_file": f"implementation/{q}/results/{q}_final.json",
                                    "source_hash": sha(qroot / "results" / f"{q}_final.json"), "format": path.suffix.lower().lstrip("."), "editable_text": path.suffix.lower() in {".svg", ".pdf"}, "status": "PASS"})
    ...
    f = {"status": "PASS" if figures else "FAIL", "figures": figures}
...
def build_submission() -> dict[str, Any]:
    ...
    copy_tree(ROOT / "data/raw", code_root / "data/raw")
    ...
    copy_tree(AUDIT, code_root / "implementation" / "final_audit")
    ...
    def copy_presentation(source: Path, public: Path, executable: Path) -> None:
        for path in sorted(source.iterdir()):
            if not path.is_file() or path.suffix.lower() not in {".pdf", ".svg", ".png", ".tiff"}: continue
            if "audit" in path.name.lower() or "alignment" in path.name.lower(): continue
            for target in (public / path.name, executable / path.name):
    ...
    for q in ("q1", "q2", "q3", "q4"):
        ...
        if figure_source.exists():
            copy_presentation(figure_source, SUBMISSION / "figures" / q, code_results / "figures")
